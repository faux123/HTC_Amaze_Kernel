...
/* HTC */
#define IS_RMNET_DBG (sdio_dbg_flag & DBG_RMNET)
#define MODULE_NAME "[RMNTSDIO]"
...
/* HTC: add IS_RMNET_DBG checking */
#define DBG(m, x...) do {			   \
		if (msm_rmnet_sdio_debug_mask & m || IS_RMNET_DBG) \
...
static int _rmnet_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (!netif_carrier_ok(dev)) {
		pr_err(MODULE_NAME "[%s] %s: channel in reset",
			dev->name, __func__);
		goto xmit_out;
	}
...
static int __init rmnet_init(void)
{
	...
	pr_info(MODULE_NAME "%s: SDIO devices[%d]\n", __func__, RMNET_DEVICE_COUNT);
