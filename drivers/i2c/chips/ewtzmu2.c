...
//#define DIF(x...) if (debug_flag) printk(KERN_DEBUG "[GYRO][PANASONIC DEBUG] " x)
...
static struct i2c_client *ewtzmu_i2c_client = NULL;
...
struct _ewtzmumid_data {
    ...
    ewtzmu_vec_t gyro;
    ewtzmu_pedo_t pedo;
    ewtzmu_vec_t linear_accel;
    ewtzmu_vec_t gravity;
...
static int EWTZMU2_Chip_Set_SampleRate (int sample_rate_state)
{
	...
	if (gpio_get_value(ewtzmumid_data.sleep_pin) == 1) {/*sleep*/
		I("sample_rate_state=%d when gyro sleep\n", sample_rate_state);
	...
	if (igyro_sleep == 1) {/*sleep*/
		I("sample_rate_state=%d reset sleep pin gpio\n", sample_rate_state);
		gpio_set_value(ewtzmumid_data.sleep_pin, 1);/*before any i2c cmd*/
		igyro_sleep = 0;
	}
...
static int EWTZMU2_ReadSensorDataFIFO(unsigned char *buf, int bufsize)
{
    ...
    if (res <= 0)
        goto exit_EWTZMU2_ReadSensorDataFIFO;
/*
    cmdbuf[0] = EWTZMU_FIFO_STS;
    cmdbuf[1] = EWTZMU_PNT_H;
    res = i2c_master_send(ewtzmu_i2c_client, cmdbuf, 2);
    if (res <= 0)
        goto exit_EWTZMU2_ReadSensorDataFIFO;

	cmdbuf[0] = EWTZMU_FIFO_STS;
    cmdbuf[1] = EWTZMU_PNT_L;
    res = i2c_master_send(ewtzmu_i2c_client, cmdbuf, 2);
    if (res <= 0)
        goto exit_EWTZMU2_ReadSensorDataFIFO;
*/
...
static int EWTZMU2_ReadPostureData(char *buf, int bufsize)
{
    ...
    sprintf(buf, "%d %d %d %d", ewtzmumid_data.yaw, ewtzmumid_data.pitch, ewtzmumid_data.roll, ewtzmumid_data.status);
...
static int EWTZMU2_ReadCaliData(char *buf, int bufsize)
{
    ...
    read_lock(&ewtzmumid_data.datalock);
    sprintf(buf, "%d %d %d %d %d %d %d", ewtzmumid_data.nm.x, ewtzmumid_data.nm.y, ewtzmumid_data.nm.z,ewtzmumid_data.na.x,ewtzmumid_data.na.y,ewtzmumid_data.na.z,ewtzmumid_data.status);
    read_unlock(&ewtzmumid_data.datalock);
    return 0;
}
...
static int EWTZMU2_ReadGyroData(char *buf, int bufsize)
{
    ...
    read_lock(&ewtzmumid_data.datalock);
    sprintf(buf, "%d %d %d", ewtzmumid_data.gyro.x, ewtzmumid_data.gyro.y, ewtzmumid_data.gyro.z);
    read_unlock(&ewtzmumid_data.datalock);
    return 0;
}
...
static int EWTZMU2_ReadPedoData(char *buf, int bufsize)
{
    ...
    read_lock(&ewtzmumid_data.datalock);
    sprintf(buf, "%ld %ld %d", ewtzmumid_data.pedo.pedo_step, ewtzmumid_data.pedo.pedo_time, ewtzmumid_data.pedo.pedo_stat);
    read_unlock(&ewtzmumid_data.datalock);
    return 0;
}
...
static int EWTZMU2_ReadMiddleControl(char *buf, int bufsize)
{
    ...
    sprintf(buf, "%d %d %d %d %d %d %d %d %d %d",
        ewtzmumid_data.controldata[EW_CB_LOOPDELAY], ewtzmumid_data.controldata[EW_CB_RUN], ewtzmumid_data.controldata[EW_CB_ACCCALI], ewtzmumid_data.controldata[EW_CB_MAGCALI],
        ewtzmumid_data.controldata[EW_CB_ACTIVESENSORS], ewtzmumid_data.controldata[EW_CB_PD_RESET], ewtzmumid_data.controldata[EW_CB_PD_EN_PARAM], ewtzmumid_data.controldata[EW_CB_GYROCALI],
        ewtzmumid_data.controldata[EW_CB_ALGORITHMLOG], ewtzmumid_data.controldata[EW_CB_UNDEFINE_1] );
...
static int EWTZMU2_ReadRotationVector(char *buf, int bufsize)
{
    ...
    read_lock(&ewtzmumid_data.datalock);
    sprintf(buf, "%d %d %d %d", ewtzmumid_data.rotationvector[0], ewtzmumid_data.rotationvector[1], ewtzmumid_data.rotationvector[2], ewtzmumid_data.rotationvector[3]);
    read_unlock(&ewtzmumid_data.datalock);
    return 0;
}
...
static int EWTZMU2_ReadLinearAccel(char *buf, int bufsize)
{
    ...
    read_lock(&ewtzmumid_data.datalock);
    sprintf(buf, "%d %d %d", ewtzmumid_data.linear_accel.x, ewtzmumid_data.linear_accel.y, ewtzmumid_data.linear_accel.z);
    read_unlock(&ewtzmumid_data.datalock);
    return 0;
}
...
static int EWTZMU2_ReadGravity(char *buf, int bufsize)
{
    ...
    read_lock(&ewtzmumid_data.datalock);
    sprintf(buf, "%d %d %d", ewtzmumid_data.gravity.x, ewtzmumid_data.gravity.y, ewtzmumid_data.gravity.z);
    read_unlock(&ewtzmumid_data.datalock);
    return 0;
}
...
static int EWTZMU2_Report_Value(void)
{
    ...
    if(atomic_read(&m_status))
    {
	   ...
	   input_report_abs(data->input_dev_compass, ABS_HAT0X, ewtzmumid_data.nm.x);/* x-axis of raw magnetic vector */
        input_report_abs(data->input_dev_compass, ABS_HAT0Y, ewtzmumid_data.nm.y);/* y-axis of raw magnetic vector */
        input_report_abs(data->input_dev_compass, ABS_BRAKE, ewtzmumid_data.nm.z);/* z-axis of raw magnetic vector */
        input_report_abs(data->input_dev_compass, ABS_WHEEL, ewtzmumid_data.status);/* status of magnetic sensor */
        report_enable = EW_REPORT_EN_COMPASS;
    }
	...
	input_report_abs(data->input_dev_compass, ABS_HAT3X, ewtzmumid_data.rotationvector[0]);/* x-axis of rotation vector */
        input_report_abs(data->input_dev_compass, ABS_HAT3Y, ewtzmumid_data.rotationvector[1]);/* y-axis of rotation vectorn */
		input_report_abs(data->input_dev_compass, ABS_TILT_X, ewtzmumid_data.rotationvector[2]);/* z-axis of rotation vector */
		input_report_abs(data->input_dev_compass, ABS_TILT_Y, ewtzmumid_data.rotationvector[3]);/* theta of rotation vector */
        report_enable = EW_REPORT_EN_COMPASS;
    }
    if(atomic_read(&la_status))
    {
        DIF( "EWTZMU2_Report_Value la_status\n ");
			input_report_abs(data->input_dev_compass, ABS_HAT1X, ewtzmumid_data.linear_accel.x);/* x-axis of linear acceleration */
        input_report_abs(data->input_dev_compass, ABS_HAT1Y, ewtzmumid_data.linear_accel.y);/* y-axis of linear acceleration */
		input_report_abs(data->input_dev_compass, ABS_TOOL_WIDTH, ewtzmumid_data.linear_accel.z);/* z-axis of linear acceleration */
        report_enable = EW_REPORT_EN_COMPASS;
    }
	...
	input_report_abs(data->input_dev_compass, ABS_HAT2X, ewtzmumid_data.gravity.x);/* x-axis of gravityr */
        input_report_abs(data->input_dev_compass, ABS_HAT2Y, ewtzmumid_data.gravity.y);/* y-axis of gravity */
        input_report_abs(data->input_dev_compass, ABS_VOLUME, ewtzmumid_data.gravity.z);/* z-axis of gravity */
        report_enable = EW_REPORT_EN_COMPASS;
    }
	...
	input_report_rel(data->input_dev_gyroscope, REL_RX, ewtzmumid_data.gyro.x);/* x-axis of gyro sensor */
        input_report_rel(data->input_dev_gyroscope, REL_RY, ewtzmumid_data.gyro.y);/* y-axis of gyro sensor */
        input_report_rel(data->input_dev_gyroscope, REL_RZ, ewtzmumid_data.gyro.z);/* z-axis of gyro sensor */
...
/*for cailibration*/
static int ewtzmu2_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)
{
    char strbuf[EW_BUFSIZE];
   ...
   /* if(!capable(CAP_SYS_ADMIN)) {
	...
	EWTZMU2_Chip_Set_SampleRate(gyro_sample_rate);
        ...
        case EW_IOCTL_WRITE_CONTROL:
			...
			if(!atomic_read(&g_status) && !atomic_read(&rv_status) && !atomic_read(&la_status) && !atomic_read(&gv_status) &&!atomic_read(&o_status) && !atomic_read(&off_status_hal))//power off
			{
			...
			else if((atomic_read(&g_status) || atomic_read(&rv_status) || atomic_read(&la_status) || atomic_read(&gv_status) || atomic_read(&o_status))  && atomic_read(&off_status_hal))//power on
			{
...
static int ewtzmu2daemon_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
       unsigned long arg)
{
    int valuebuf[4];
    int calidata[7];
    ...
    switch (cmd) {
		...
		DIF("sensordata 2:%d %d ,%d %d, %d %d, %d\n",
			databuf[6],databuf[7],databuf[8],databuf[9],databuf[10],databuf[11], databuf[12]);
            if (copy_to_user(data, &(databuf[1]), databuf[0])) {
                retval = -EFAULT;
                goto err_out;
            }
...
static int ewtzmu2hal_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)
{
    int controlbuf[EW_CB_LENGTH];
    char strbuf[EW_BUFSIZE];
    int pedoparam[EW_PD_LENGTH];
    ...
    switch (cmd) {
       ...
       case EWHAL_IOCTL_SET_PEDOPARAM:
		...
		I("Gyro AP on: controldata_active_sensor=0x%x\n",
			...
			/*if(!atomic_read(&g_status) && !atomic_read(&o_status) && !atomic_read(&off_status_hal))*///power off
			//if((!atomic_read(&g_status)) && (!atomic_read(&o_status)))
			...
			//else if((atomic_read(&g_status) || atomic_read(&o_status))  && atomic_read(&off_status_hal))//power on
			//else if((atomic_read(&g_status)) || (atomic_read(&o_status)))//power on
...
static int ewtzmu2_input_init(struct ewtzmu_i2c_data *data)
{
    ...
    if (!data->input_dev_compass) {
        err = -ENOMEM;
        E(
               "ewtzmu2_input_init: Failed to allocate input device\n");
        goto exit_input_dev_alloc_failed;
    }
    set_bit(EV_ABS, data->input_dev_compass->evbit);
    /* yaw */
    input_set_abs_params(data->input_dev_compass, ABS_RX, 0, (360*10), 0, 0);
    /* pitch */
    input_set_abs_params(data->input_dev_compass, ABS_RY, -(180*10), (180*10), 0, 0);
    /* roll */
    input_set_abs_params(data->input_dev_compass, ABS_RZ, -(90*10), (90*10), 0, 0);
    /* status of orientation sensor */
    input_set_abs_params(data->input_dev_compass, ABS_RUDDER, 0, 5, 0, 0);

    /* x-axis of raw acceleration and the range is -2g to +2g */
    input_set_abs_params(data->input_dev_compass, ABS_X, -(1000*2), (1000*2), 0, 0);
    ...
    input_set_abs_params(data->input_dev_compass, ABS_Y, -(1000*2), (1000*2), 0, 0);
    ...
    input_set_abs_params(data->input_dev_compass, ABS_Z, -(1000*2), (1000*2), 0, 0);
    ...
    input_set_abs_params(data->input_dev_compass, ABS_HAT0X, -(1000*3), (1000*3), 0, 0);
    /* y-axis of raw magnetic vector and the range is -3g to +3g */
    input_set_abs_params(data->input_dev_compass, ABS_HAT0Y, -(1000*3), (1000*3), 0, 0);
    /* z-axis of raw magnetic vector and the range is -3g to +3g */
    input_set_abs_params(data->input_dev_compass, ABS_BRAKE, -(1000*3), (1000*3), 0, 0);
    ...
    input_set_abs_params(data->input_dev_compass, ABS_HAT1X, -(1000*2), (1000*2), 0, 0);
    /* y-axis linear acceleration and the range is -2g to +2g */
    input_set_abs_params(data->input_dev_compass, ABS_HAT1Y, -(1000*2), (1000*2), 0, 0);
    /* z-axis linear acceleration and the range is -2g to +2g */
    input_set_abs_params(data->input_dev_compass, ABS_TOOL_WIDTH, -(1000*2), (1000*2), 0, 0);

	/* x-axis gravity and the range is -2g to +2g */
    input_set_abs_params(data->input_dev_compass, ABS_HAT2X, -(1000*2), (1000*2), 0, 0);
    /* y-axis gravity and the range is -2g to +2g */
    input_set_abs_params(data->input_dev_compass, ABS_HAT2Y, -(1000*2), (1000*2), 0, 0);
    /* z-axis gravity and the range is -2g to +2g */
    input_set_abs_params(data->input_dev_compass, ABS_VOLUME, -(1000*2), (1000*2), 0, 0);
    //data->input_dev_compass->name = "ewtzmu2_compass";
...
// developers need to add ewtzmu2 i2c address at i2c_board_info structure
// under arch/arm/chipset-name (ex:samsung/arch/arm/mach-s5pv210 for N-S)
// Ex:
// static struct i2c_board_info i2c_devs1[] __initdata = {
//        {
//                I2C_BOARD_INFO("ewtzmu2", i2c_address),
//        },
//};
...
static int __devinit ewtzmu2_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!(data = kmalloc(sizeof(struct ewtzmu_i2c_data), GFP_KERNEL))) {
        err = -ENOMEM;
        goto exit;
    }
	...
	if (data->pdata->config_gyro_diag_gpios != NULL)
		ewtzmumid_data.config_gyro_diag_gpios = data->pdata->config_gyro_diag_gpios;
	atomic_set(&off_status_hal, 1);
	EWTZMU2_Power_Off();
...
static int __init ewtzmu2_init(void)
{
    ...
    ewtzmumid_data.controldata[EW_CB_LOOPDELAY] = EW_DEFAULT_POLLING_TIME;  // Loop Delay
    ewtzmumid_data.controldata[EW_CB_RUN] =           1;   // Run
    ewtzmumid_data.controldata[EW_CB_ACCCALI] =       0;   // Start-AccCali
    ewtzmumid_data.controldata[EW_CB_MAGCALI] =       1;   // Start-MagCali
    ewtzmumid_data.controldata[EW_CB_ACTIVESENSORS] = 0;   // Active Sensors
    ewtzmumid_data.controldata[EW_CB_PD_RESET] =      0;   // Pedometer not reset
    ewtzmumid_data.controldata[EW_CB_PD_EN_PARAM] =   0;   // Disable parameters of Pedometer
