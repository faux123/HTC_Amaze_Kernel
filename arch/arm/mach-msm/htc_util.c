...
void htc_MPSS_LPASS_stat_show(void)
{
	...
	uint64_t MPSS_total_sleep_time = 0;
	uint64_t LPASS_total_sleep_time = 0;
	...
	MPSS_total_sleep_time = htc_get_MPSS_total_sleep_time();
	LPASS_total_sleep_time = htc_get_LPASS_total_sleep_time();
	...
	if (MPSS_active_timestamp == gMPSS_active_timestamp && MPSS_sleep_timestamp == gMPSS_sleep_timestamp) {
		if (MPSS_active_timestamp > MPSS_sleep_timestamp)
			printk(KERN_INFO "MPSS avtive (%llums)\n", MPSS_total_sleep_time);
		else
			printk(KERN_INFO "MPSS sleep (%llums)\n", MPSS_total_sleep_time);
	} else {
		printk(KERN_INFO "MPSS running (%llums)\n", MPSS_total_sleep_time);
	}
	...
	if (LPASS_active_timestamp == gLPASS_active_timestamp && LPASS_sleep_timestamp == gLPASS_sleep_timestamp) {
		if (LPASS_active_timestamp > LPASS_sleep_timestamp)
			printk(KERN_INFO "LPASS avtive (%llums)\n", LPASS_total_sleep_time);
		else
			printk(KERN_INFO "LPASS sleep (%llums)\n", LPASS_total_sleep_time);
	} else {
		printk(KERN_INFO "LPASS running (%llums)\n", LPASS_total_sleep_time);
	}
...
void htc_PM_monitor_init(void)
{
	...
	BUG_ON(!prev_proc_stat || !curr_proc_delta || !task_ptr_array);
