...
/*============================================================*/
/* HTC: include files */
#include "../../../../drivers/mmc/host/msm_sdcc_8x60.h"
#include <linux/rtc.h>
...
#define LAW_DEBUG(x...)  if (sdio_dbg_flag & DBG_LAWDATA) pr_info(x)
#define MEMC_DEBUG(x...) if (sdio_dbg_flag & DBG_MEMCPY)  pr_info(x)

#undef  pr_debug
#define pr_debug(x...)   if ((sdio_dbg_flag & DBG_ALDEBUG) \
			|| flag_aldbg) pr_info(x)
...
struct sdio_al_debug {
	...
	u8 debug_lpm_on;
	u8 debug_data_on;
	u8 debug_al_msg;	/* HTC: add for pr_debug */
	u8 padding;			/* HTC: struct padding */
...
static int sdio_al_wake_up(struct sdio_al_device *sdio_al_dev,
			   u32 not_from_int, struct sdio_channel *ch); /* HTC add ch arg */
...
static atomic_t first_time; /* 1: means sdio_al_print_info is running, 2: done */
DECLARE_COMPLETION(com_print_info);
...
atomic_t sdio_write_run_state = {0}; /* 1: means sdio_write is running, 0: done or not run */
DECLARE_COMPLETION(com_sdio_write);
/* --SSD_RIL */
/* ++SSD_RIL: Mars_Lin@20110809: For debug TX pending data */
int sdio_tx_pending_count = 0;/* count the tx pending, and enable kernel penic if pending over 1 min */
/* --SSD_RIL */
/*============================================================*/
...
/* HTC: memcpy checking function */
static int sdio_al_memcpy_fromio_wrapper(struct sdio_func *func, void *dst,
	unsigned int addr, int count, struct memc_counter *mcnt);
static int sdio_al_memcpy_toio_wrapper(struct sdio_func *func, unsigned int addr,
	void *src, int count, struct memc_counter *mcnt);
...
/* ++SSD_RIL: Mars_Lin@20110809: For sdio_write COMPLETION. */
void sdio_write_start(void);
void sdio_write_end(void);
void sdio_write_wait_complete(void);
/* --SSD_RIL */
/*============================================================*/
...
static int read_mailbox(struct sdio_al_device *sdio_al_dev, int from_isr)
{
	...
	static struct memc_counter memc = { caller: __func__ }; /* HTC */
	unsigned int rxpending_ch_mask = 0; /* HTC: the result of rxpending */
	unsigned int txpending_ch_mask = 0; /* HTC: the result of txpending */
	...
	ret = sdio_al_memcpy_fromio_wrapper(func1, mailbox,
			HW_MAILBOX_ADDR, sizeof(*mailbox), &memc);
	...
	if ((rx_notify_bitmask == 0) && (tx_notify_bitmask == 0) &&
		...
		if (is_inactive_time_expired(sdio_al_dev)) {
			...
			if (atomic_read(&reader_count) || atomic_read(&writer_count)) {
				LPM_DEBUG(MODULE_NAME ": reader_count %d, "
					"writer_count %d "
...
//++SSD_RIL: Mars_Lin@20110630: print log every 15 seconds
			if (sdio_pending_log_count >= SDIO_PENDING_DATA_LOG_MAX_COUNT) {
				pr_info(MODULE_NAME ":Pending data for card %d "
						...
						sdio_al_dev->card->host->index, txpending_ch_mask, rxpending_ch_mask);
/* ++SSD_RIL: Mars_Lin@20110809: For debug TX pending data */
				if (sdio_al_dev->card->host->index == 4) {
					if (txpending_ch_mask == 0) {
						sdio_tx_pending_count = 0;
					} else {
						sdio_tx_pending_count++;
						pr_info(MODULE_NAME ": %s: sdio_tx_pending_count=[%d]\n", __func__, sdio_tx_pending_count);
						if (sdio_tx_pending_count >= 4 && (sdio_dbg_flag & DBG_MDM)) {
							/* trigger kernel penic */
							pr_err(MODULE_NAME ": %s: trigger kernel panic\n", __func__);
							BUG_ON(1);
						}
					}
				}
...
//--SSD_RIL
		...
		/* ++SSD_RIL: Mars_Lin@20110818: For debug TX pending data */
		else {
			if (txpending_ch_mask == 0)
				sdio_tx_pending_count = 0;
		}
...
static u32 check_pending_rx_packet(struct sdio_channel *ch, u32 eot)
{
	...
	if (eot & (1<<ch->rx_pipe_index)) {
		...
		if (p == NULL) {
			pr_err(MODULE_NAME ":failed to allocate item for rx_pending list."
					    " rx_avail=%d, rx_pending=%d.\n", rx_avail, rx_pending);
			new_packet_size = 0;
			goto exit_err;
		}
...
static int sdio_ch_write(struct sdio_channel *ch, const u8 *buf, u32 len)
{
	...
	if (remain_bytes) {
		/* CMD53 */
		if (blocks) {
			ret = sdio_al_memcpy_toio_wrapper(ch->func, PIPE_TX_FIFO_ADDR,
...
static int sdio_al_bootloader_setup(void)
{
	...
	ret = sdio_al_memcpy_fromio_wrapper(func1,
				 bootloader_dev->sdioc_boot_sw_header,
				 SDIOC_SW_HEADER_ADDR,
				 sizeof(struct peer_sdioc_boot_sw_header), &memc);
	if (ret) {
				      ...
				      struct peer_sdioc_sw_header *header)
{
	...
	ret = sdio_al_memcpy_fromio_wrapper(sdio_al_dev->card->sdio_func[0], header,
			SDIOC_SW_HEADER_ADDR, sizeof(*header), &memc);
	if (ret) {
...
static int read_sdioc_channel_config(struct sdio_channel *ch)
{
	...
	ret = sdio_al_memcpy_fromio_wrapper(ch->func, sw_mailbox,
			SDIOC_SW_MAILBOX_ADDR, sizeof(*sw_mailbox), &memc);
	if (ret) {
...
static void sdio_func_irq(struct sdio_func *func)
{
	...
	if (sdio_al_dev->is_ok_to_sleep)
		sdio_al_wake_up(sdio_al_dev, 0, NULL); /* HTC */
	else
		 ...
		 void (*notify)(void *priv, unsigned ch_event))
{
	...
	*ret_ch = NULL; /* default */
	...
	ret = sdio_al_wake_up(sdio_al_dev, 1, ch); /* HTC */
	if (ret)
		goto exit_err;
...
int sdio_read(struct sdio_channel *ch, void *data, int len)
{
	...
	ret = sdio_al_memcpy_fromio_wrapper(ch->func, data, PIPE_RX_FIFO_ADDR, len, &memc);
	...
	if (wakeup_from_interrupt) {
		if (mach_vigor && ((sdio_dbg_flag & DBG_LPM)
			...
			pr_info(MODULE_NAME ": card%d, <%s> %s"
				...
				sdio_al_dev->card->host->index, ch->name, __func__, ch->total_tx_bytes,
			ch->total_rx_bytes, ch->write_avail, ch->read_avail,
			tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
			tm.tm_min, tm.tm_sec);
...
int sdio_write(struct sdio_channel *ch, const void *data, int len)
{
	...
	dbg_dump_buf(ch->name, "SDIO_AL->W", data, size, CHECK_DBG_FLAG); /* HTC */
	sdio_release_host(sdio_al_dev->card->sdio_func[0]);
...
static void sdio_al_sdio_remove(struct sdio_func *func)
{
	...
	/* HTC: wait for print_info finish */
	if(atomic_read(&first_time) == 1) {
		pr_info(MODULE_NAME ": %s - wait for print_info complete...\n", __func__);
		wait_for_completion(&com_print_info);
		pr_info(MODULE_NAME ": continuie %s\n", __func__);
	}

	/* ++SSD_RIL: Mars_Lin@20110809: For sdio_write COMPLETION. */
	sdio_write_wait_complete();
	/* --SSD_RIL */
...
static void sdio_al_print_info(void)
{
	...
	/* static atomic_t first_time; */ /* HTC: declare as global */
	char buf[50];

	if (atomic_read(&first_time) >= 1) /* HTC: >= 1, origin == 1 */
		return;
	...
	for (j = 0 ; j < MAX_NUM_OF_SDIO_DEVICES ; ++j) {
		...
		if (sdio_al_verify_dev(sdio_al_dev, __func__))
			continue; /* HTC: continue should be enough */
...
//++SSD_RIL: Mars_Lin verify device before call sdio_claim_host
		if (sdio_al_verify_dev(sdio_al_dev, __func__))
		{
			pr_err(MODULE_NAME ": %s - fail verify dev before call sdio_claim_host",
					__func__);
			continue;
		}
		pr_info(MODULE_NAME "%s before claim_host and sdio_memcpy_fromio\n", __func__);
//--SSD_RIL
...
//++SSD_RIL: Mars_Lin verify device before call sdio_release_host
		if (sdio_al_verify_dev(sdio_al_dev, __func__))
		{
			pr_err(MODULE_NAME ": %s - fail verify dev before call sdio_release_host",
					__func__);
			continue;
		}
		pr_info(MODULE_NAME "%s after claim_host and sdio_memcpy_fromio\n", __func__);
//--SSD_RIL
	...
	for (j = 0 ; j < MAX_NUM_OF_SDIO_DEVICES ; ++j) {
		...
		pr_info(MODULE_NAME ": %s before sdio_claim_host func1\n", __func__); /* HTC */
		sdio_claim_host(func1);
...
exit_print: /* HTC: synchronize with remove card */
	pr_info(MODULE_NAME ": %s complete\n", __func__);
	atomic_set(&first_time, 2);
	complete(&com_print_info);
}
...
static int __init sdio_al_init(void)
{
	...
	pr_info(MODULE_NAME ": %s() - get_kernel_flag=0x%x, default sdio_dbg_flag=0x%x\n",
			__func__, kernel_flag, sdio_dbg_flag);
	sdio_dbg_flag |= kernel_flag & BIT16 ? DBG_DMUX   : 0;
	sdio_dbg_flag |= kernel_flag & BIT17 ? DBG_RMNET  : 0;
	sdio_dbg_flag |= kernel_flag & BIT18 ? DBG_CMUX   : 0;
	sdio_dbg_flag |= kernel_flag & BIT19 ? (DBG_DATA | DBG_LPM) : 0;

	sdio_dbg_flag |= kernel_flag & BIT20 ? DBG_CTL    : 0;
	sdio_dbg_flag |= kernel_flag & BIT21 ? (DBG_ALDEBUG | DBG_LAWDATA) : 0;
	sdio_dbg_flag |= kernel_flag & BIT22 ? DBG_MEMCPY : 0;
	sdio_dbg_flag |= kernel_flag & BIT23 ? DBG_RPC    : 0;
	...
	unsigned int addr, int count, struct memc_counter *mcnt)
{
	...
	++mcnt->in;
	MEMC_DEBUG(MODULE_NAME ":before sdio_memcpy_fromio %s-%d.\n",
			mcnt->caller, mcnt->in);
	ret = sdio_memcpy_fromio(func, dst, addr, count);
	++mcnt->out;
	MEMC_DEBUG(MODULE_NAME ":after  sdio_memcpy_fromio %s-%d. complete\n",
			mcnt->caller, mcnt->out);
	return ret;
}
	...
	void *src, int count, struct memc_counter *mcnt)
{
	int ret;
	++mcnt->in;
	MEMC_DEBUG(MODULE_NAME ":before sdio_memcpy_toio %s-%d.\n",
			mcnt->caller, mcnt->in);
	ret = sdio_memcpy_toio(func, addr, src, count);
	++mcnt->out;
	MEMC_DEBUG(MODULE_NAME ":after  sdio_memcpy_toio %s-%d. complete\n",
			mcnt->caller, mcnt->out);
	return ret;
}
...
void sdio_write_start(void)
{
	atomic_set(&sdio_write_run_state, 1);
	init_completion(&com_sdio_write);
}
void sdio_write_end(void)
{
	atomic_set(&sdio_write_run_state, 0);
	complete(&com_sdio_write);
}
void sdio_write_wait_complete(void)
{
	if (atomic_read(&sdio_write_run_state) == 1) {
		pr_info(MODULE_NAME ": %s: wait for sdio_write complete\n", __func__);
		wait_for_completion(&com_sdio_write);
