...
#define IS_CTL_DBG (sdio_dbg_flag & DBG_CTL)
...
#if defined(DEBUG) /* HTC: add IS_CTL_DBG checking */
#define D_DUMP_BUFFER(prestr, cnt, buf) \
do { \
	if (msm_sdio_ctl_debug_mask & MSM_SDIO_CTL_DUMP_BUFFER || IS_CTL_DBG) { \
		int i; \
		pr_info("%s", prestr); \
		for (i = 0; i < cnt; i++) \
			pr_info("%.2x", buf[i]); \
...
#define D(x...) \
do { \
	if (msm_sdio_ctl_debug_mask & MSM_SDIO_CTL_DEBUG || IS_CTL_DBG) \
		pr_info(x); \
} while (0)
...
int sdio_ctl_open(struct inode *inode, struct file *file)
{
	...
	if (!sdio_ctl_inited) {
		pr_info("%s with !sdio_ctl_inited\n", __func__);
		return -EIO;
	}
	...
	if (!sdio_ctl_devp) {
		pr_info("%s with !sdio_ctl_devp\n", __func__);
		return -ENODEV;
	}

	pr_info("%s called on sdioctl%d device\n", __func__, sdio_ctl_devp->id);
...
static int sdio_ctl_probe(struct platform_device *pdev)
{
	...
	sdio_ctl_inited = 1;
	pr_info("SDIO Control Port Driver Initialized.\n");
	return 0;
