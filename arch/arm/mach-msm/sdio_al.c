...
#include <mach/sdiodbg-8x60.h>
#include <mach/htc_ril_misc.h>
#include <mach/board_htc.h>
/*============================================================*/
/* HTC: definitions */
#define SDIO_AL_POLL_TRY_WAIT_USER_IRQ_FROM_MDM 5000
#define is_chnum_valid(m) (m >= 0 && m < SDIO_AL_MAX_CHANNELS)? 1 : 0

#define LAW_DEBUG(x...) \
	do { \
		if (sdio_dbg_flag & DBG_LAWDATA) \
			pr_info(x); \
	} while (0)
#define MEMC_DEBUG(x...) \
	do { \
		if (sdio_dbg_flag & DBG_MEMCPY) \
			pr_info(x); \
	} while (0)
...
#define pr_debug(x...) \
	do { \
		if ((sdio_dbg_flag & DBG_ALDEBUG) || flag_aldbg) \
			pr_info(x); \
	} while (0)

/*
#define DATA_DEBUG(x...) if ((sdio_dbg_flag & DBG_DATA) \
			|| (sdio_al->debug.debug_data_on)) pr_info(x)
#define LPM_DEBUG(x...)  if ((sdio_dbg_flag & DBG_LPM) \
			|| (sdio_al->debug.debug_lpm_on)) pr_info(x)
*/
...
#define DATA_DEBUG(x, y...)						\
	do {								\
		if ((sdio_dbg_flag & DBG_LPM) || (sdio_al->debug.debug_data_on))\
			pr_info(y);					\
		sdio_al_log(x, y);					\
	} while (0)
...
#define LPM_DEBUG(x, y...)						\
	do {								\
		if ((sdio_dbg_flag & DBG_DATA) || (sdio_al->debug.debug_lpm_on))\
			pr_info(y);					\
		sdio_al_log(x, y);					\
	} while (0)
...
/* ++SSD_RIL: Mars_Lin@20110630: print log every 15 seconds */
#define SDIO_PENDING_DATA_LOG_MAX_COUNT 500
/* --SSD_RIL */
...
struct sdio_al_device {
	...
	/* HTC */
	unsigned int rxpending_ch_mask;
	unsigned int txpending_ch_mask;
	unsigned long active_duration;
	unsigned long active_duration_total;
};
...
#ifdef DEBUG  /* start ifdef DEBUG */
    bool flag_aldbg = true;
#else
    bool flag_aldbg = false;
#endif /* end ifdef DEBUG */

const int tp_tm_ms = 300000;
struct timer_list tp_timer;

/* ++SSD_RIL: Mars_Lin@20110630: print log every 15 seconds */
int sdio_pending_log_count = 0;
/* --SSD_RIL */
/*============================================================*/
/* HTC: functions prototype */
static void ask_reading_mailbox(struct sdio_al_device *sdio_al_dev);
static void dbg_dump_buf(const char *name,
		u8 *prestr, const u8 *buf, int len, int check_dbg_flag);
enum { /* enum for dbg_dump_buf */
	NO_NEED_TO_CHECK_DBG_FLAG,
	CHECK_DBG_FLAG,
};

/* HTC: TP statistic functions */
static void tp_timer_handler(unsigned long data);
static void tp_start_timer(void);
static void print_statistic(struct sdio_al_device *dev,
		struct sdio_channel *ch, int n_print_time);

/* HTC: provide interface for user space to enable sdio debug */
static ssize_t sdio_dbg_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t sdio_dbg_store(struct device *dev,
				   ...
				   int is_vote_for_sleep)
{
	...
	if (is_vote_for_sleep) {
		...
		/* HTC */
		sdio_al_dev->active_duration += jiffies;
		sdio_al_dev->active_duration_total +=
		...
		if (mach_vigor) {
			...
			LPM_DEBUG(sdio_al_dev->dev_log, MODULE_NAME ": %s - sdio vote against sleep",
				  __func__);
		}
		wake_lock(&sdio_al_dev->wake_lock);
		sdio_al_dev->active_duration = -jiffies; /* HTC */
	}
			  ...
			  struct mmc_host *host)
{
	...
	if (mach_vigor) {/* HTC: change the log level */
		...
		LPM_DEBUG(sdio_al_dev->dev_log, MODULE_NAME  ":Inactivity timer expired."
			" Going to sleep\n");
	}
	/* Stop mailbox timer */
	...
	for (i = 0; i < SDIO_AL_MAX_CHANNELS; i++) {
		...
		if ((ch->state != SDIO_CHANNEL_STATE_OPEN) &&
			...
			pr_debug(MODULE_NAME  ":continue for channel %8.6s in"
					" state %d\n", ch->name, ch->state);
			continue;
		}
...
static int read_mailbox(struct sdio_al_device *sdio_al_dev, int from_isr)
{
	...
	/* HTC */
	sdio_al_dev->rxpending_ch_mask = 0;
	sdio_al_dev->txpending_ch_mask = 0;
	...
	for (i = 0; i < SDIO_AL_MAX_CHANNELS; i++) {
		...
		if ((ch->state == SDIO_CHANNEL_STATE_CLOSED) &&
			(read_avail > 0)) {
			sdio_al_loge(sdio_al_dev->dev_log, MODULE_NAME
				 ":%s: Invalid read_avail 0x%x, for CLOSED ch %8.6s\n",
		...
		/* HTC: print out read pending channel info */
		if ((read_avail | old_read_avail) && !rx_notify_bitmask)
			sdio_al_dev->rxpending_ch_mask |= (1<<ch->num);
	}
	...
	for (i = 0; i < SDIO_AL_MAX_CHANNELS; i++) {
		...
		if (strcmp(ch->name, "SDIO_SMEM")) {
			...
			/* HTC: get the ch num of tx pending channel */
			if ((new_write_avail < ch->ch_config.max_tx_threshold) &&
				!tx_notify_bitmask)
				sdio_al_dev->txpending_ch_mask |= (1<<ch->num);
		}
	}
	/* notify clients */
	for (i = 0; i < SDIO_AL_MAX_CHANNELS; i++) {
	...
	if ((rx_notify_bitmask == 0) && (tx_notify_bitmask == 0) &&
		...
		if (is_closing)
			...
			/* HTC: if block: make sure no any reader/writer before sleeping */
			if (atomic_read(&reader_count) || atomic_read(&writer_count)) {
				LPM_DEBUG(sdio_al_dev->dev_log,
					MODULE_NAME ": reader_count %d, "
					"writer_count %d "
		...
		if (!rx_notify_bitmask && !tx_notify_bitmask) {
			...
			if (sdio_pending_log_count >= SDIO_PENDING_DATA_LOG_MAX_COUNT) {
				...
				pr_info(MODULE_NAME ":Pending data for card %d "
						...
						sdio_al_dev->txpending_ch_mask,
						sdio_al_dev->rxpending_ch_mask);
#else
				DATA_DEBUG(sdio_al_dev->dev_log, MODULE_NAME
						":Pending data for card %d "
						"tx_ch=0x%x, rx_ch=0x%x\n",
						sdio_al_dev->card->host->index,
						sdio_al_dev->txpending_ch_mask,
						sdio_al_dev->rxpending_ch_mask);
#endif
				sdio_pending_log_count = 0;
			} else {
				sdio_pending_log_count++;
			}
			/* --SSD_RIL */
		}
...
static void boot_worker(struct work_struct *work)
{
	...
	/* Added by HTC: Lloyd for throughput */
	...
	pr_info(MODULE_NAME ":Do polling until MDM is up for card %d, at most 50 sec..\n",
			sdio_al_dev->card->host->index); /* Added by HTC */
	for (i = 0; i < SDIO_AL_POLL_TRY_WAIT_USER_IRQ_FROM_MDM; ++i) {
		...
		if (is_user_irq_enabled(sdio_al_dev, func_num)) {
			...
			pr_info(MODULE_NAME ":Got user_irq from MDM for card %d\n",
					sdio_al_dev->card->host->index); /* Added by HTC */
			goto done;
		}
		sdio_al_release_mutex(sdio_al_dev, __func__);
		...
		if ((i % 50) == 0)	/* Added by HTC */
			pr_info(MODULE_NAME ":polling MDM to wait for user_irq "
					"for card %d, try: [%d/%d]\n", sdio_al_dev->card->host->index,
					(i / 50), (SDIO_AL_POLL_TRY_WAIT_USER_IRQ_FROM_MDM / 50));
	}
...
static int sdio_ch_write(struct sdio_channel *ch, const u8 *buf, u32 len)
{
	...
	if (ret != 0) {
		sdio_al_loge(ch->sdio_al_dev->dev_log, MODULE_NAME ":%s: "
				"sdio_write_cmd54 failed for channel %8.6s\n",
				__func__, ch->name);
				      ...
				      struct peer_sdioc_sw_header *header)
{
	...
	for (i = 0; i < SDIO_AL_MAX_CHANNELS; i++) {
		...
		/* Modified by HTC */
		if (strcmp(header->channel_names[i], "RMNT") != 0)
			ch->min_write_avail = DEFAULT_MIN_WRITE_THRESHOLD;
		else
			ch->min_write_avail = 1600;
		/*------------------------------------------------*/
		...
		sdio_al_logi(sdio_al_dev->dev_log, MODULE_NAME ":Channel=%8.6s, "
				"state=%d\n", ch->name,	ch->state);
	}
...
static int read_sdioc_channel_config(struct sdio_channel *ch)
{
	...
	CLOSE_DEBUG(sdio_al_dev->dev_log, MODULE_NAME ":ch %8.6s "
...
static int open_channel(struct sdio_channel *ch)
{
	...
	pr_debug(MODULE_NAME ":channel %8.6s is in OPEN state now\n", ch->name);
			   ...
			   u32 not_from_int, struct sdio_channel *ch)
{
	...
	struct mmc_host *host = sdio_al_dev->host;
	struct timespec ts; struct rtc_time tm; /* HTC */
	...
	if (not_from_int) {
		...
		if (!ch || !ch->name || !mach_vigor) /* HTC */
			...
			LPM_DEBUG(sdio_al_dev->dev_log, MODULE_NAME
				...
				sdio_al_dev->host->index, ch->name,
				ch->total_tx_bytes,
	...
	LPM_DEBUG(sdio_al_dev->dev_log, MODULE_NAME ":GPIO mdm2ap_vddmin=%d\n",
	...
	if (sdio_al->pdata->get_mdm2ap_status() == 0) {
		...
		/* Added by HTC: retry get_mdm2ap_status() */
		int out_of_vddmin = 0;
		pr_info(MODULE_NAME ": %s Poll MDM2AP_VDDMIN again\n", __func__);
		time_to_wait = jiffies + msecs_to_jiffies(1000);
		while (time_before(jiffies, time_to_wait)) {
			if (sdio_al->pdata->get_mdm2ap_status()) {
				out_of_vddmin = 1;
		...
		if (!out_of_vddmin) /* HTC: retry and still got 0 */
			pr_info(MODULE_NAME ": %s get_mdm2ap_status() is 0\n", __func__);
	}
	...
	if (ret) {
		sdio_al_loge(sdio_al_dev->dev_log, MODULE_NAME ": "
				"sdio_enable_func() err=%d\n", -ret);
		/* HTC */
		if (sdio_dbg_flag & DBG_MDM) {
			int gpio = sdio_al->pdata->mdm2ap_errfatal_gpio;
			if (gpio && (gpio_get_value(gpio) == 0)) {
				pr_err(MODULE_NAME ":trigger kernel panic\n");
				BUG_ON(1);
			}
		}
		goto error_exit;
	...
	LPM_DEBUG(sdio_al_dev->dev_log,
			MODULE_NAME "=== Finished Wake up sequence"
		 ...
		 void (*notify)(void *priv, unsigned ch_event))
{
	...
	*ret_ch = NULL; /* default */
	...
	if ((ch->state != SDIO_CHANNEL_STATE_IDLE) &&
		(ch->state != SDIO_CHANNEL_STATE_CLOSED)) {
		sdio_al_loge(sdio_al_dev->dev_log, MODULE_NAME ":Wrong ch %8.6s "
				"state %d\n", name, ch->state);
		ret = -EPERM;
		goto exit_err;
	}
	...
	*ret_ch = ch;
	...
	ret = open_channel(ch);
	if (ret) {
		sdio_al_loge(sdio_al_dev->dev_log, MODULE_NAME ":sdio_open %8.6s "
				"err=%d\n", name, -ret);
		goto exit_err;
	}

	CLOSE_DEBUG(sdio_al_dev->dev_log, MODULE_NAME ":sdio_open %8.6s "
							"completed OK\n", name);
	if (sdio_al_dev->lpm_chan == INVALID_SDIO_CHAN) {
		if (sdio_al->sdioc_major == PEER_SDIOC_OLD_VERSION_MAJOR) {
			if (!ch->is_packet_mode) {
				sdio_al_logi(sdio_al_dev->dev_log, MODULE_NAME
						":setting channel %8.6s as "
						"lpm_chan\n", name);
				sdio_al_dev->lpm_chan = ch->num;
			}
		} else {
			sdio_al_logi(sdio_al_dev->dev_log, MODULE_NAME ": "
					"setting channel %8.6s as lpm_chan\n",
					name);
			sdio_al_dev->lpm_chan = ch->num;
		}
	}
		...
		struct sdio_channel *ch)
{
	...
	pr_debug(MODULE_NAME ":%s: wk_func enabled on ch %8.6s\n",
			__func__, ch->name);
...
static int channel_close(struct sdio_channel *ch, int flush_flag)
{
	...
	if (ch->state != SDIO_CHANNEL_STATE_OPEN) {
		sdio_al_loge(sdio_al_dev->dev_log,
				MODULE_NAME ":%s: ch %8.6s is not in "
				"open state (%d)\n",
	...
	ch->state = SDIO_CHANNEL_STATE_CLOSED;
	CLOSE_DEBUG(sdio_al_dev->dev_log, MODULE_NAME ":%s: Ch %8.6s closed "
				"successfully\n", __func__, ch->name);
...
int sdio_write_avail(struct sdio_channel *ch)
{
	...
	if (ch->state != SDIO_CHANNEL_STATE_OPEN) {
		sdio_al_loge(ch->sdio_al_dev->dev_log, MODULE_NAME ":%s: "
				"channel %8.6s state is not open (%d)\n",
				__func__, ch->name, ch->state);
		return -ENODEV;
	}
...
static int sdio_read_from_closed_ch(struct sdio_channel *ch, int len)
{
	...
	if (ret) {
		sdio_al_loge(ch->sdio_al_dev->dev_log,
				MODULE_NAME ":ch %8.6s: %s err=%d, len=%d\n",
...
static int sdio_read_internal(struct sdio_channel *ch, void *data, int len)
{
	...
	if (len == 0) {
		sdio_al_loge(&sdio_al->gen_log, MODULE_NAME ":channel %8.6s trying"
				" to read 0 bytes\n", ch->name);
		return -EINVAL;
	}
	...
	/* HTC: change the following code from !ch->is_open checking to
       sdio_memcpy_fromio checking. Remove BUG_ON and add HTC solution.
    ...
    if (unlikely(sdio_al_dev->is_ok_to_sleep)) {
		sdio_al_loge(sdio_al_dev->dev_log, MODULE_NAME ":%s: called "
				"when is_ok_to_sleep is set for ch %8.6s, len=%d,"
		...
		/* HTC: Prevent from calling sdio_memcpy_fromio
			without turning on clock, Don't call BUG_ON
			for this case. htc: recover this case instead */
		ret = sdio_al_wake_up(sdio_al_dev, 1, ch);
		if (ret)
			goto exit;
    } else
		restart_inactive_time(sdio_al_dev);
	/* BUG_ON(sdio_al_dev->is_ok_to_sleep); QCT original*/


	if ((ch->state != SDIO_CHANNEL_STATE_OPEN) &&
			(ch->state != SDIO_CHANNEL_STATE_CLOSING)) {
		sdio_al_loge(sdio_al_dev->dev_log, MODULE_NAME ":%s wrong "
				"channel %8.6s state %d\n",
				__func__, ch->name, ch->state);
		ret = -EINVAL;
		goto exit;
	}

	DATA_DEBUG(sdio_al_dev->dev_log, MODULE_NAME ":start ch %8.6s read %d "
	...
	if (len > ch->read_avail) {
		sdio_al_loge(sdio_al_dev->dev_log, MODULE_NAME ":ERR ch %8.6s: "
	...
	if (ret) {
		pr_err(MODULE_NAME ": %s GPIO mdm2ap_vddmin=%d\n",
				__func__, sdio_al->pdata->get_mdm2ap_status()); /* HTC */
		sdio_al_loge(sdio_al_dev->dev_log, MODULE_NAME ":ch %8.6s: "
	...
	ch->total_rx_times += 1; /* HTC */
	DATA_DEBUG(sdio_al_dev->dev_log, MODULE_NAME ":end ch %8.6s read %d "
			"avail %d total %d.\n", ch->name, len,
	...
	if (wakeup_from_interrupt) {
		if (mach_vigor && ((sdio_dbg_flag & DBG_LPM)
			...
			pr_info(MODULE_NAME ": card%d, <%8.6s> %s"
				...
				sdio_al_dev->host->index, ch->name,
				__func__, ch->total_tx_bytes,
			ch->total_rx_bytes, ch->write_avail, ch->read_avail,
			tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
			tm.tm_min, tm.tm_sec);

			/* force to print out the raw data for power issues */
			dbg_dump_buf(ch->name, "SDIO_AL<-R", data, len, NO_NEED_TO_CHECK_DBG_FLAG);
			print_buf_already = 1;
		}
		wakeup_from_interrupt = 0;
	}

	/* HTC: dump buf for debugging */
	if (0 == print_buf_already) /* print_buf_already 1: already print */
		dbg_dump_buf(ch->name, "SDIO_AL<-R", data, len, CHECK_DBG_FLAG);
...
int sdio_read(struct sdio_channel *ch, void *data, int len)
{
	...
	if (ch->state == SDIO_CHANNEL_STATE_OPEN) {
		...
		sdio_al_loge(ch->sdio_al_dev->dev_log, MODULE_NAME
				":%s: Invalid channel %8.6s state %d\n",
				__func__, ch->name, ch->state);
	}
	return -ENODEV;
}
...
int sdio_write(struct sdio_channel *ch, const void *data, int len)
{
	...
	DATA_DEBUG(sdio_al_dev->dev_log, MODULE_NAME ":start ch %8.6s write %d "
	...
	size = len; /* HTC: size = len without ROUND_UP */

	ret = sdio_ch_write(ch, data, len);
	if (ret) {
		sdio_al_loge(sdio_al_dev->dev_log, MODULE_NAME ":sdio_write "
				"on channel %8.6s err=%d\n", ch->name, -ret);
		goto exit;
	}

	ch->total_tx_bytes += len;
	ch->total_tx_times += 1; /* HTC */
	DATA_DEBUG(sdio_al_dev->dev_log, MODULE_NAME ":end ch %8.6s write %d "
			"avail %d total %d.\n", ch->name, len,
	...
	/* HTC */
	dbg_dump_buf(ch->name, "SDIO_AL->W", data, size, CHECK_DBG_FLAG);
...
static int __devinit msm_sdio_al_probe(struct platform_device *pdev)
{
	...
	/* HTC: create device attriubte for sdio debug */
	if (pdev->name)
		pr_info(MODULE_NAME ":%s platform=%s%d\n", __func__,
				pdev->name, pdev->id);
	if (pdev->dev.init_name)
		pr_info(MODULE_NAME ":%s device=%s\n", __func__,
				pdev->dev.init_name);
	if (device_create_file(&pdev->dev,&dev_attr_sdio_dbg))
	    pr_info(MODULE_NAME"Warning: sdio_al attribute can't be created\n");
...
static void sdio_al_close_all_channels(struct sdio_al_device *sdio_al_dev)
{
	...
	for (j = 0; j < SDIO_AL_MAX_CHANNELS; j++) {
		...
		if (ch->state == SDIO_CHANNEL_STATE_OPEN) {
			...
			pr_debug(MODULE_NAME ": %s: skip sdio_close() ch %8.6s"
					" (state=%d)\n", __func__,
					ch->name, ch->state);
		}
	}
...
static int init_channels(struct sdio_al_device *sdio_al_dev)
{
	...
	for (i = 0; i < SDIO_AL_MAX_CHANNELS; i++) {
		...
		if (!sdio_al_dev->channel[i].pdev) {
			sdio_al_loge(sdio_al_dev->dev_log, MODULE_NAME
					":NULL platform device for ch %8.6s",
					sdio_al_dev->channel[i].name);
		...
		/* HTC: check whether init_channel is finished for sdio_tty */
		else if (0 == strcmp("SDIO_DUN", sdio_al_dev->channel[i].name))
			sdio_dun_ch_init = 1;
	}
...
static void sdio_al_print_info(void)
{
	...
	ex_charm_dump_mdm_related_gpio_status(); /* HTC: dump mdm/ap gpio */

	sdio_al_loge(&sdio_al->gen_log, MODULE_NAME ": %s - SDIO DEBUG INFO\n",
			__func__);

	sdio_al_loge(&sdio_al->gen_log, MODULE_NAME ": GPIO mdm2ap_vddmin=%d\n",
				sdio_al->pdata->get_mdm2ap_status());
...
static int __init sdio_al_init(void)
{
	...
	/* HTC: sdio debug flag by kernelflag
		dmux 10000,  rmnet 20000, cmux 40000, sdio_al 680000
		ctrl 100000, rpc 800000 */
	...
	/* HTC: get kernel flag to set debug flag */
	kernel_flag = get_kernel_flag();
	pr_info(MODULE_NAME ": %s() - get_kernel_flag=0x%x,"
			"default sdio_dbg_flag=0x%x\n",
			__func__, kernel_flag, sdio_dbg_flag);

	sdio_dbg_flag |= kernel_flag & KERNEL_FLAG_RIL_DBG_DMUX ? DBG_DMUX : 0;
	sdio_dbg_flag |= kernel_flag & KERNEL_FLAG_RIL_DBG_RMNET ? DBG_RMNET : 0;
	sdio_dbg_flag |= kernel_flag & KERNEL_FLAG_RIL_DBG_CMUX ? DBG_CMUX : 0;
	sdio_dbg_flag |= kernel_flag & KERNEL_FLAG_RIL_DBG_DATA_LPM ? (DBG_DATA | DBG_LPM) : 0;

	sdio_dbg_flag |= kernel_flag & KERNEL_FLAG_RIL_DBG_CTL ? DBG_CTL : 0;
	sdio_dbg_flag |= kernel_flag & KERNEL_FLAG_RIL_DBG_ALDEBUG_LAWDATA ? (DBG_ALDEBUG | DBG_LAWDATA) : 0;
	sdio_dbg_flag |= kernel_flag & KERNEL_FLAG_RIL_DBG_MEMCPY ? DBG_MEMCPY : 0;
	sdio_dbg_flag |= kernel_flag & KERNEL_FLAG_RIL_DBG_RPC ? DBG_RPC : 0;

	radio_flag = get_radio_flag();
	pr_info(MODULE_NAME ": %s() - get_radio_flag=0x%x\n",
			__func__, radio_flag);
	sdio_dbg_flag |= (radio_flag & 0x01) ? DBG_MDM : 0;
	...
	tp_timer.expires = jiffies + msecs_to_jiffies(50000);
	add_timer(&tp_timer);
...
/*============================================================*/
/* HTC: functions */
/* HTC: print out the buffer, the string size must smaller than DATA_SIZE  */
/* #define ENABLE_DUMP_TXT_BUF */
static void dbg_dump_buf(const char *name,
		u8 *prestr, const u8 *buf, int len, int check_dbg_flag)
{
    const int DATA_SIZE = 96; /* 96/3 = 32 = bytes would be printed */
    unsigned char bin_buf[DATA_SIZE];
	int bin_size = DATA_SIZE;
    int i, min = len, ret = 0,
		count = 0, limit = DATA_SIZE;
#ifdef ENABLE_DUMP_TXT_BUF
	unsigned char txt_buf[DATA_SIZE >> 1];
	int txt_size = DATA_SIZE >> 1;
#endif

	/* sanity checking and init */
	if (!name || !buf || !prestr)
		return;
	if (CHECK_DBG_FLAG == check_dbg_flag
			&& false == (sdio_dbg_flag & DBG_LAWDATA))
		return;

	/* get the limit of the buf size */
	min = len > bin_size ? bin_size : min ;

	memset(bin_buf, 0, bin_size);
#ifdef ENABLE_DUMP_TXT_BUF
	memset(txt_buf, 0, txt_size);
#endif

	printk(MODULE_NAME);
	PRINTRTC; /* print out timestamp */

	/* fill in the bin/txt buffer */
    for (i = 0; i < min; i++) {
		ret = snprintf(bin_buf + count, limit, "%02x ", buf[i]);
#ifdef ENABLE_DUMP_TXT_BUF
		if (i < txt_size-1)
			txt_buf[i] = isprint(buf[i])? buf[i] : '.';
#endif
		/* bin_buf: every time we got 3 chars until buf full */
		if (likely(limit > 3)) {
			count += ret;
			limit -= ret;
		} else {
			/* final round, no more buf, deal with the bin_buf */
			if (count >= bin_size)
				bin_buf[bin_size-1] = '\0';
			else
				bin_buf[count] = '\0';
#ifdef ENABLE_DUMP_TXT_BUF
			/* add EOS to txt_buf */
			if (i >= txt_size-1)
				txt_buf[txt_size-1] = '\0';
			else
				txt_buf[i] = '\0';
#endif
		}
			...
			const char *buf, size_t size)
{
	...
	pr_info(MODULE_NAME "%s set sdio_dbg_flag as 0x%08x\n",
			__func__, sdio_dbg_flag);
...
/*
 *  HTC: Statistic of each channels every n seconds.
 *
...
static void tp_timer_handler(unsigned long data)
{
	...
	if (!sdio_al) {
		pr_info(MODULE_NAME ": Stop tp_timer_handler Success!\n");
		return;
	}

	/* go through every sdio devices */
	for (i = 0; i < MAX_NUM_OF_SDIO_DEVICES; i++) {
		if (sdio_al->devices[i] == NULL)
			continue;
		dev = sdio_al->devices[i];
		/* go throuhg every channels on the device */
		for (k = 0; k < SDIO_AL_MAX_CHANNELS; k++) {
			if (&dev->channel[k] == NULL)
				continue;
			ch = &dev->channel[k];

			print_statistic(dev, ch, k);
		}
		/* setup next statistics for active duration */
		dev->active_duration_total = 0;
	}
 ...
 *  HTC: print_statistic()
 *
 *  @dev: device that contains card info
 *  @ch: channel that contains channel info
 *  @n_print_time: 0 print out current UTC,
 *        1 print out, tx/rx only.
 */
static void print_statistic(struct sdio_al_device *dev,
		struct sdio_channel *ch, int n_print_time)
{
	struct timespec ts;
	struct rtc_time tm;
	int print_time = n_print_time == 0 ? 1 : 0; /* 0->1, 1->0 */

	if (!dev || !dev->card || !dev->card->host || !ch || !ch->name)
		return;
	if (0 == strlen(ch->name))
		return;
	...
	if (print_time) {
		...
		pr_info(MODULE_NAME ": %d <%-8.6s> t=%d, r=%d, tt=%d, rt=%d,"
				" %d-%d, ad%d-%ld(%02d-%02d %02d:%02d:%02d)\n",
				dev->card->host->index, ch->name,
				/* ch statistics */
				ch->total_tx_bytes, ch->total_rx_bytes,
				ch->total_tx_times, ch->total_rx_times,
				/* dev info */
				dev->txpending_ch_mask, dev->rxpending_ch_mask,
				/* active duration */
				dev->card->host->index, dev->active_duration_total,
				/* time */
...
/*
 * HTC: tp_start_timer: add timer and call tp_timer_handler when time's up
 *
 */
static void tp_start_timer(void)
{
    tp_timer.expires = jiffies + msecs_to_jiffies(tp_tm_ms);
    add_timer(&tp_timer);
}
...
static inline void print_statistic(struct sdio_al_device *dev,
		struct sdio_channel *ch, int n_print_time)
{
}
